...
function UnavailableMedicine() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/api/dashboard/EmergencyMedicine/${id}`, {
  ...
  return (
    <>
      ...
      {jsonData ? (
				<>
				<h2 className="head-medicineTaken">Emergency Medicine</h2>
        <div className="medicine-item2">
				{/* <pre>{JSON.stringify(jsonData, null, 2)}</pre> */
					console.log(jsonData)}
				{
				jsonData.allEmergencyMedicalRecords.map((item) => (
					<div key={item._id}>
