...
const DoctorDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await axios
        .get(`http://localhost:4000/api/v1/doctor/${spaciality}`, {
  ...
  const registerData = async (doctorId) => {
    await axios
      ...
      .then((res) => {
        navigate(`/doctor/appointment/confirmation/${res.data.newRecord._id}`);
